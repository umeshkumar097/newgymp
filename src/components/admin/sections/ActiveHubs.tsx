      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8 px-4">
         <div className="space-y-3">
            <div className="flex items-center space-x-3 text-brand-green">
               <Zap size={18} />
               <span className="text-[10px] font-black uppercase tracking-[0.3em]">Network Pulse</span>
            </div>
            <h2 className="text-3xl md:text-4xl font-black text-slate-900 uppercase tracking-tighter italic leading-none">Active <span className="text-brand-green underline decoration-slate-100 underline-offset-8">Hubs</span></h2>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest italic">India's most premium fitness portal inventory.</p>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        {filteredGyms.length === 0 ? (
           <div className="col-span-full p-28 bg-white border border-slate-100 border-dashed rounded-[2.5rem] flex flex-col items-center justify-center opacity-20 text-center">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.5em] text-slate-500 italic">No Active Hubs Discovered</p>
           </div>
        ) : (
          filteredGyms.map((gym: any) => (
            <div key={gym.id} className="bg-white border border-slate-200/60 rounded-[2rem] p-10 space-y-10 shadow-xl shadow-slate-200/20 relative overflow-hidden group hover:shadow-2xl transition-all duration-700 ring-1 ring-slate-50">
               ...
               {gym.isPaused && (
                  <div className="absolute inset-0 bg-white/95 backdrop-blur-xl z-20 flex flex-col items-center justify-center space-y-8 animate-in fade-in duration-500">
                     ...
                     <div className="text-center space-y-2 px-12">
                        <h4 className="text-2xl font-black text-slate-900 uppercase tracking-tighter italic leading-none">Hub Deactivated</h4>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Shielded from the public marketplace.</p>
                     </div>
                     <button 
                        onClick={() => handleTogglePause(gym.id, true)}
                        className="bg-slate-900 text-white px-8 py-4 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-brand-green hover:text-slate-950 transition-all shadow-xl active:scale-95 italic"
                     >
                        Release Access
                     </button>
                  </div>
                  ...
                  <div className="flex items-center space-x-6">
                     <div className="w-24 h-24 rounded-2xl bg-slate-50 border border-slate-100 overflow-hidden shadow-sm relative group-hover:scale-105 transition-transform duration-700 ring-1 ring-slate-50">
                        {gym.imageUrls && gym.imageUrls[0] ? (
                           <Image src={gym.imageUrls[0]} alt="" width={96} height={96} className="w-full h-full object-cover opacity-80 group-hover:opacity-100" />
                        ) : (
                     ...
                     <div className="space-y-1.5">
                        <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight italic leading-none">{gym.name}</h3>
                        <p className="text-[10px] font-bold text-slate-400 flex items-center tracking-widest uppercase italic">
                           <MapPin size={12} className="mr-2 text-brand-green/30" />
                           {gym.location}
                        </p>
                        <div className="flex items-center space-x-4 pt-1">
                           <div className="flex items-center text-[9px] font-black text-slate-300 uppercase tracking-[0.2em]">
                              <Clock size={10} className="mr-1.5 text-slate-200" />
                              <span>{gym.openingTime || "06 AM"} - {gym.closingTime || "10 PM"}</span>
                           </div>
                           <div className="flex items-center text-[9px] font-black text-slate-300 uppercase tracking-[0.2em]">
                              <Calendar size={10} className="mr-1.5 text-slate-200" />
               ...
               <div className="grid grid-cols-3 gap-6 border-t border-slate-50 pt-10">
                  <div className="space-y-1.5">
                     <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] flex items-center italic">
                        <Zap size={10} className="mr-1.5 text-brand-green/30" />
                        Reach
                     </p>
                     <p className="text-2xl font-black text-slate-900 tracking-tighter italic leading-none">{gym._count?.bookings || "0"} <span className="text-[9px] text-slate-300 font-bold ml-1 uppercase">Passes</span></p>
                  </div>
                  <div className="space-y-1.5 border-l border-slate-50 pl-6">
                     <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] flex items-center italic">
                        <Star size={10} className="mr-1.5 text-blue-500/30" />
                        Rating
                     </p>
                     <p className="text-2xl font-black text-slate-900 tracking-tighter italic leading-none">{gym.avgRating ? gym.avgRating.toFixed(1) : "5.0"}</p>
                  </div>
                  <div className="space-y-1.5 text-right">
                     <p className="text-[9px] font-black text-brand-green uppercase tracking-[0.2em] italic underline decoration-brand-green/10 underline-offset-4 text-right">Net Share</p>
               ...
               <div className="flex gap-3 pt-2 relative z-10">
                  <button 
                    ...
                    className="flex-1 bg-slate-50 border border-slate-100 hover:bg-white hover:border-slate-200 text-slate-400 hover:text-slate-900 font-black py-4 rounded-xl text-[9px] uppercase tracking-[0.2em] transition-all flex items-center justify-center space-x-2 group italic shadow-sm"
                  >
                     <BarChart size={14} className="text-slate-300 group-hover:text-brand-green transition-colors" />
                     <span>Analytics</span>
                  </button>
                  <button 
                    ...
                    className="flex-1 bg-slate-50 border border-slate-100 hover:bg-white hover:border-slate-200 text-slate-400 hover:text-slate-900 font-black py-4 rounded-xl text-[9px] uppercase tracking-[0.2em] transition-all flex items-center justify-center space-x-2 group italic shadow-sm"
                  >
                     <User size={14} className="text-slate-300 group-hover:text-blue-500 transition-colors" />
                     <span>Review</span>
                  </button>
                  <button 
