      ...
      <aside className="w-72 bg-white border-r border-slate-200/60 flex flex-col hidden md:flex sticky top-0 h-screen shadow-sm">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "flex items-center space-x-3 px-6 py-4 rounded-2xl transition-all font-black text-[10px] uppercase tracking-[0.2em] italic",
                  isActive 
                    ? "bg-slate-900 text-white shadow-xl shadow-slate-200/50 scale-[1.02]" 
                    : "text-slate-400 hover:text-slate-900 hover:bg-slate-50"
                )}
              >
                <Icon size={18} className={cn(isActive ? "text-brand-green" : "text-slate-300")} />
                <span>{item.name}</span>
              </Link>
            );
          })}
        ...
        <div className="p-6 border-t border-slate-100">
          <button 
            ...
            className="flex items-center space-x-3 px-6 py-4 w-full text-slate-400 hover:text-red-500 text-[10px] uppercase font-black tracking-[0.2em] transition-all group italic"
          >
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-x-hidden relative">
        <header className="h-24 bg-white/80 backdrop-blur-3xl border-b border-slate-200/40 flex items-center justify-between px-10 sticky top-0 z-[100] shadow-sm">
          <div className="flex flex-col">
             <h2 className="text-xl font-black italic text-slate-900 leading-none">GOD-EYE HUB</h2>
             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Enterprise Monitoring Protocol</p>
          </div>
          ...
          <div className="flex items-center space-x-6">
            ...
            <div className="flex items-center space-x-4 pl-4 border-l border-slate-100">
               <div className="text-right hidden sm:block">
                  <p className="text-xs font-black text-slate-900 italic leading-none">Vipul Kumar</p>
                  <p className="text-[9px] font-black text-brand-green uppercase tracking-widest mt-1">Super Admin</p>
               </div>
